...
function AddressBookEntryForModification(props){
    ...
    const updateAddress = async() =>{
        try{
            await apiClient.put(`/users/${address && address.user_id}/addresses/${address && address.id}`,{"address":{"address": addressValue}});
        }catch(error){
    ...
    const deleteAddress = async() =>{
        try{
            await apiClient.delete(`/users/${address && address.user_id}/addresses/${address && address.id}`);
        }catch(error){
            console.log("ERROR", error);
        }
    };
